package
{
   ...
   public class SceneRoot extends Sprite
   {
      ...
      public function makePath(param1:Point, param2:Point) : void
      {
         ...
         var _loc8_:* = NaN;
         var _loc9_:* = null;
         _loc5_ = this.findCollisionPoints(param1,param2);
         if(_loc5_.length == 0)
         {
         ...
         _loc6_ = _loc5_[0];
         _loc7_ = _loc5_[1];
         _loc8_ = Math.sqrt((_loc7_.x - _loc6_.x) * (_loc7_.x - _loc6_.x) + (_loc7_.y - _loc6_.y) * (_loc7_.y - _loc6_.y));
         if(_loc8_ < MINIMUM_AB_DIST)
         {
            return;
         }
         _loc9_ = this.findPointC(_loc6_,_loc7_);
         ...
         if(this.m_newObject)
         {
            ...
            this.m_objectExitPt = _loc7_;
            this.makePath(param1,_loc9_);
            this.makePath(_loc9_,_loc7_);
            if(_loc7_ != param2)
            {
      ...
      public function setHalloweenSceneTime(param1:int, param2:Boolean) : void
      {
         ...
         _loc3_ = 0;
         var _loc6_:*;
         var _loc7_:* = _loc6_ = _loc5_ = this.m_sceneTime;
         switch(_loc7_)
         {
            case SCENE_TIME_DAY:
      ...
      protected function updateFireflies() : void
      {
         ...
         _loc2_ = Math.floor(Math.random() * 100);
         if(_loc2_ > 95)
         {
            if(this.m_fireflyList.length < MAX_FIREFLIES)
            {
               _loc4_ = Math.floor(Math.random() * 935);
               _loc5_ = Math.floor(Math.random() * 600);
               _loc6_ = new Firefly();
               _loc6_.x = _loc4_;
               _loc6_.y = _loc5_;
               this.m_fireflyList.push(_loc6_);
               if(this.m_effectsContainer)
               {
                  this.m_effectsContainer.addChild(_loc6_);
               }
            }
         }
         _loc3_ = this.m_fireflyList.length - 1;
         while(_loc3_ >= 0)
         {
            this.m_fireflyList[_loc3_].update();
            if(this.m_fireflyList[_loc3_].hasExpired())
            {
               if(Boolean(this.m_effectsContainer) && Boolean(this.m_effectsContainer.contains(this.m_fireflyList[_loc3_])))
               {
                  this.m_effectsContainer.removeChild(this.m_fireflyList[_loc3_]);
               }
               this.m_fireflyList[_loc3_].destroy();
               this.m_fireflyList.splice(_loc3_,1);
            }
            _loc3_--;
         }
      }
